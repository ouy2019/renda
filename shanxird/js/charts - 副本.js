...
$(document).ready(function () {
   ...
   // localStorage.setItem()
            ...
            success: function (data) {
                ...
                if (data.length != 0 && data.length != null) {
                    ...
                    $('.dynli').click(function () {
                        var loading = layer.msg('努力加载中...', {icon: 16, shade: 0.3, time:0});
    ...
    //表格数据
    function tablePage(){
      console.log(localStorage.getItem('tableid'),'表格数据'); //274910628018262016
      ...
      $.ajax({
          ...
          success: function (msg){
           //console.log(msg,'---表格数据---');
            ...
            console.log(convertD,'---表格数据--941');
            // return
            layui.use('table', function(){
                var table = layui.table;//第一个实例
                table.render({
                  elem: '#layuiTable',
                //   height: convertD.length>10? 10*40 : convertD.length*40, 
                  height: 'full-200', 
                //   cols:[convertH[0]],
                  cols:convertH,
                  data:convertD,
    ...
    function convertHeaders (headers, config) { //解析表格表头数据
        ...
        const heads = headers.map(headerRow => {
            return headerRow.reduce((prev, cur, idx) => {
                // 左锁定
                ...
                if(sizeWidth && sizeWidth.size){
                    obj.width = sizeWidth.size;
                }
                prev.push(obj);
                return prev;
            }, []);
        })
       console.log(heads,'--表头部--h');
        return heads;
    }
    function convertData(data,config,convertTou){ //解析表格数据
        ...
        data.forEach((item,index)=>{
            var convertObj={};
            // 左锁定
            const leftLock =  config && config.find(reportViewConfigs =>
            reportViewConfigs.rowIndex == index && reportViewConfigs.type === 'LEFT_LOCK_COL');
            // 右锁定
            const rightLock =  false;
            item.map((items,indexs)=>{
                // console.log(convertTou,'convertTou---')
                if(!items.index) return;
                const key = 'a' + items.index;
                convertObj[key] = {};
                convertObj[key] = items.text;
                // convertObj[key].field = items.text;
                // convertObj[key].rowspan = items.rowSpan;
                // convertObj[key].colspan = items.colSpan;
                // convertObj[key].width = parseInt(items.width, 10)+100;
                // convertObj[key].align = 'center';
            })
            convertObj.fixed = leftLock ? 'left' : 'right';
